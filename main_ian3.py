...
#        if test_mode:
#            df = pd.read_csv('Files/optimal_features_calibration.csv', sep=',') 
...
#            pred, prob = run_model(trained_model, X, X_holdout, y)
#            test_mode_stat = report(name + ", holdout", y_holdout, pred, prob)
#            df_out = pd.DataFrame(pred,columns=['Predicted Class'])
#            df_out.to_csv('Files/predicted_class.csv', sep=',')   
